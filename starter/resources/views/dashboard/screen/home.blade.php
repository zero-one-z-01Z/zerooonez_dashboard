    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="row g-6">
            ...
            @foreach($numbers as $number)
                @include('dashboard.home-parts.numbers',$number)
            @endforeach
                @isset($avg)
                    @include('dashboard.home-parts.average-day',$avg)
                @endisset


                @isset($sales)
                    @include('dashboard.home-parts.sales-summary',$sales)
                @endisset

                @isset($total_earn)
                    @include('dashboard.home-parts.total_earn',$total_earn)
                @endisset

                @isset($circle_chart)
                    @include('dashboard.home-parts.circle_chat')
                @endisset


                @isset($list_compare)
                    @include('dashboard.home-parts.list_compare',$list_compare)
                @endisset

                @include('dashboard.home-parts.graph-year')

                @isset($progress)
                    @include('dashboard.home-parts.progress',$progress)
                @endisset
