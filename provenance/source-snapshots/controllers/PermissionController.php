...
class PermissionController extends Controller
{
    ...
    public function permissionList()
    {
        ...
        return app(\ZeroOneZ\Dashboard\Services\Dashboard\ResourceRegistry::class)->permissionGroups($permissions);
    }
    ...
    public function index()
    {
        ...
        return view('zerooonez-dashboard::screen.table_view',compact('data','permissions'));
    }
    ...
    {
        $requestData = $request->only('name_ar','name_en');
        $role = Role::create($requestData);
        $data = $request->except('name_ar','name_en');
        foreach ($data as $key => $value){

            $check = $request->input($key);
            if($check){
                $permission = Permission::firstOrCreate(['key_name' => $key]);
                if($permission){
