...
/**
 * Converts a translated toolbar label to visible text, including labels supplied
 * as a DOM element by a legacy page script.
 * @param {*} value translation string or DOM-like node.
 * @returns {string} safe text for jQuery.text().
 */
export function toolbarLabel(value) {
    if (value == null) return '';
    if (typeof value === 'object' && typeof value.textContent === 'string') return value.textContent;
    return String(value);
}
...
export function setUpDatatable() {
    ...
    $('#import_text').text(toolbarLabel(window.import_text));
    $('#export_text').text(toolbarLabel(window.export_text));
    $('#delete_all_text').text(toolbarLabel(window.delete_all_text));
    $('#add_new_item_text').text(toolbarLabel(window.addNewItem));
