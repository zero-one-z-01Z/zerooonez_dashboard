...
//        ],'select2'=>true,'url'=>route('admin.admins.list_super'),'key_name'=>'admin_company','value_name'=>'name'];
    ...
    public function login(){
        ...
        return view('dashboard.screen.auth.login');
    }
    ...
    public function forget_password(){
        ...
        return view('dashboard.screen.auth.forget_password');
    }
    ...
    public function reset_password($token){
        ...
        return view('dashboard.screen.auth.reset_password',compact('token'));
    }
    ...
    public function send_forget_password(Request $request)
    {
        ...
        $admin = Admin::query()->where('email', $request->email)->where('active', true)->first();
        if (! $admin) return back()->with('success', __('admin.success'));
    ...
    {
        $data = $request->validate(['name' => ['required', 'string', 'max:255'], 'email' => ['required', 'email', 'unique:admins,email'], 'phone' => ['required', 'string', 'max:255', 'unique:admins,phone'], 'role_id' => ['required', 'exists:roles,id'], 'password' => ['nullable', 'confirmed', 'min:8']]);
    ...
    {
        $admin = $this->model()->findOrFail($request->integer('id'));
        abort_if($admin->super && ! admin_user()?->super, 403);
        $data = $request->validate(['name' => ['required', 'string', 'max:255'], 'email' => ['required', 'email', 'unique:admins,email,'.$admin->id], 'phone' => ['required', 'string', 'max:255', 'unique:admins,phone,'.$admin->id], 'role_id' => ['required', 'exists:roles,id'], 'new_password' => ['nullable', 'confirmed', 'min:8']]);
        if (! empty($data['new_password'])) $data['password'] = Hash::make($data['new_password']);
    ...
    public function delete($id)
    {
        ...
        abort_if($admin->super || $admin->is(admin_user()), 403);
        $admin->delete();
